...
function* getItemSaga(action) {
    try {
        ...
        let res = yield callApi(types.HTTP_READ, '', localStorage.getItem('token'))
        console.log('res', res);
        if (res.errorMessage) {
...
function* addItemSaga(action) {
    try {
        ...
        let res = yield callApi(types.HTTP_CREATE, '', { name: name ,token})
...
function* updateItemSaga(action) {
    try {
        ...
        let name = action.payload.name
        let token =localStorage.getItem('token')
        let res = yield callApi(types.HTTP_UPDATE, id, { name: name,token })
        if (res.errorMessage) {
...
function* loginUserSaga(action) {
    try {
        console.log('actiom saga',action.payload);
        let res = yield callApi(types.HTTP_CREATE, 'login', action.payload)
        if (res.errorMessage) {
...
function* registerUserSaga(action) {
    try {
        let res = yield callApi(types.HTTP_CREATE, 'register', action.payload)
        if (res.errorMessage) {
