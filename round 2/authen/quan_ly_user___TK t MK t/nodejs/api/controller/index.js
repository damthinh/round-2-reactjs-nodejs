...
exports.registerUser = async (req, res) => {
    try {
        ...
        if (checkUserName) {
            ...
            await userModel.create({ userName,email, passWord: mahoaPassWord })
...
exports.getUser = async (req, res) => {
    try {
        let listItem = await userModel.find().select('-passWord')
        res.send(listItem)
    } catch (error) {
        res.send({ errorMessage: error.message })
    }
...
exports.updateUser = async (req, res) => {
    try {
        ...
        let updateUser
        if (role===1) {
            updateUser = await userModel.findByIdAndUpdate(id,{role:'1'},{new:true})
        }else if (role===2) {
            updateUser = await userModel.findByIdAndUpdate(id,{role:'2'},{new:true})
        }
        else{
           return res.send({ errorMessage: 'loi role' })
        }
        res.send(updateUser)
    } catch (error) {
        res.send({ errorMessage: error.message })
