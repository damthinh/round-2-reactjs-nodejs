...
exports.updateConnectnBook = async (req, res) => {
    try {
        ...
        for (let i in id_Books) {
            if (id_Books[i]) {
                let getBooks = await modelBooks.findByIdAndUpdate(id_Books[i],{ $addToSet: { id_Author: { $each: id_Author } } },{new:false})
                for(let j in getBooks.id_Author){
                    if (!id_Author.includes(`${getBooks.id_Author[j]}`)) {
                        await modelBooks.findByIdAndUpdate(id_Books[i],{ $pull: { id_Author:  getBooks.id_Author[j] } })
                        await modelAuthor.findByIdAndUpdate(getBooks.id_Author[j], { $pull: { id_Books: { $in: id_Books } } })
                    }
                }
            }
        } 
        ...
        res.send({ message: "update Connect ok" })
    } catch (error) {
...
exports.updateConnectnAuthor = async (req, res) => {
    try {
        ...
        for (let i in id_Author) {
            if (id_Author[i]) {
                let getAuthor = await modelAuthor.findByIdAndUpdate(id_Author[i],{ $addToSet: { id_Books: { $each: id_Books } } },{new:false})
                for(let j in getAuthor.id_Books){
                    if (!id_Books.includes(`${getAuthor.id_Books[j]}`)) {
                        await modelAuthor.findByIdAndUpdate(id_Author[i],{ $pull: { id_Books:  getAuthor.id_Books[j] } })
                        await modelBooks.findByIdAndUpdate(getAuthor.id_Books[j], { $pull: { id_Author: { $in: id_Author } } })
                    }
                }
            }
        } 
        ...
        res.send({ message: "update Connect Author ok" })
    } catch (error) {
        res.send({ errorMessage: error.message })
    }
}
...
// exports.updateConnectnAuthor = async (req, res) => {
//     try {
//         let { id_Author, id_Books ,id_Books_Delete} = req.body
//         for (let i in id_Author) {
//             if (id_Author[i]) {
//                 await modelAuthor.findByIdAndUpdate(id_Author[i], { $pull: { id_Books: { $in: id_Books_Delete } } })
//                 await modelAuthor.findByIdAndUpdate(id_Author[i], { $addToSet: { id_Books: id_Books } })
//             }
//         } for (let i in id_Books) {
//             if (id_Books[i]) {
...
//             }
//         }
//         for (let i in id_Books_Delete) {
//             if (id_Books_Delete[i]) {
//                 await modelBooks.findByIdAndUpdate(id_Books_Delete[i], { $pull: { id_Author: { $in: id_Author } } })
//             }
//         }
//         res.send({ message: "update Connect nAuthor ok" })
//     } catch (error) {
//         res.send({ errorMessage: error })
//     }
// }
